...
const DeployEngine = (() => {
  ...
  function getDeployMethod(type, policyId) {
    ...
    // When a deployment proxy is configured, upgrade DEF/EXO/PV from PS-only
    // to their InvokeCommand routes — the proxy bypasses browser CORS.
  ...
  // Map SPO PowerShell parameter values to Graph enum values where they differ.
  ...
  // Pull params block from either {cmdlet, parameters} or steps[0]
  ...
  const SPO_GRAPH_MAP = {
    'SPO01': function (raw) {
      var p = spoParams(raw);
      return {
        sharingCapability: spoSharingCapability(p.SharingCapability || 'ExistingExternalUserSharingOnly'),
      };
    },
    'SPO02': function (raw) {
      var p = spoParams(raw);
    ...
    'SPO03': function (raw) {
      var p = spoParams(raw);
      return {
        anonymousLinkExpirationRestrictionDays: Number(p.RequireAnonymousLinksExpireInDays || 30),
      };
    },
    'SPO07': function () {
      return {
    ...
    'SPO14': function () {
      // Block apps where device enforcement is missing — restrict unmanaged sync app
  ...
  function setDeploymentProxy(url) {
    try {
      ...
      if (!normalised) {
        // Empty input → clear override and fall back to default.
        localStorage.removeItem(PROXY_KEY);
      } else if (normalised === DEFAULT_PROXY_URL) {
        localStorage.removeItem(PROXY_KEY);
      } else {
        localStorage.setItem(PROXY_KEY, normalised);
      }
    } catch (e) { /* ignore */ }
  }
  ...
  function hasDeploymentProxy() {
    return !!getDeploymentProxy();
  }
  ...
  async function callInvokeCommand(cmdletName, parameters, method) {
    ...
    const directUrl = (isCompliance ? COMPLIANCE_INVOKE_BASE : EXO_INVOKE_BASE)
      + acct.tenantId + '/InvokeCommand';
    ...
    let url, opts;
    if (proxy) {
      // Proxy mode: send target URL + token + body in a single POST.
      // Reference proxy in proxy/index.js validates and forwards.
      url = proxy + '/invoke';
      opts = {
        method: 'POST',
