...
const Assessment = (() => {
  ...
  function renderStep1(container) {
    ...
    for (const [group, fws] of Object.entries(fwGroups)) {
      ...
      for (const fw of fws) {
        const meta = (AppState.get('frameworkMeta') || {})[fw];
        const badge = meta && meta.version ? `<span class="fw-version-badge">${escHtml(meta.version)}</span>` : '';
        const dot = meta && meta.status === 'outdated'
          ? '<span class="status-dot outdated" title="Newer version available"></span>' : '';
        html += `<label class="group-fw-item">
          <input type="checkbox" ${sel.has(fw) ? 'checked' : ''} onchange="Assessment.toggleFw('${escHtml(fw)}')">
          <span>${fw}</span>${badge}${dot}
        </label>`;
      }
      html += `</div></div>`;
    }
  ...
  function exportAssessment() {
    var data = {
      _meta: { type: 'M365-Assessment-Export', version: '1.0', exportDate: new Date().toISOString() },
    ...
    a.href = url;
    a.download = 'M365-Assessment-' + new Date().toISOString().slice(0, 10) + '.json';
    a.click();
    ...
    input.onchange = function (e) {
      ...
      reader.onload = function (ev) {
        try {
          ...
          if (!data._meta || data._meta.type !== 'M365-Assessment-Export') {
            showToast('Invalid assessment file format');
            return;
          }
          if (!Array.isArray(data.selectedFrameworks)) {
            showToast('Invalid format: missing selectedFrameworks');
            return;
          }
          if (AppState.get('selectedFrameworks').size > 0) {
            if (!confirm('This will replace your current assessment data. Continue?')) return;
          }
          AppState.set('selectedFrameworks', new Set(data.selectedFrameworks));
          AppState.set('checkStatus', data.checkStatus || {});
          AppState.set('assessmentStep', data.assessmentStep || 1);
          renderStepBar();
          renderCurrentStep();
          renderSidebar();
          showToast('Imported: ' + data.selectedFrameworks.length + ' frameworks, ' +
            Object.keys(data.checkStatus || {}).length + ' statuses');
        } catch (err) {
