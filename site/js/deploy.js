...
const DeployEngine = (() => {
  ...
  async function deploySinglePolicy(id) {
    ...
    return _deployWithRawJson(pol, rawPolicy);
  }

  // Deploy with pre-modified JSON (used by PreDeployConfig after user customisation)
  async function deploySinglePolicyWithJson(id, rawPolicy) {
    const pol = AppState.get('policies').find(function (p) { return p.id === id; });
    if (!pol) return { success: false, error: 'Policy not found' };
    return _deployWithRawJson(pol, rawPolicy);
  }

  async function _deployWithRawJson(pol, rawPolicy) {
    ...
    if (!await runPreflight(method)) {
      setDeploymentStatus(pol.id, 'failed', 'Preflight failed for ' + method);
      return { success: false, error: 'Preflight failed' };
    }

    setDeploymentStatus(pol.id, 'deploying', 'Deploying...');
    ...
    try {
      ...
      if (method === DEPLOY_METHOD.GRAPH) {
        ...
        if (payload.calls.length === 0) {
          setDeploymentStatus(pol.id, 'failed', 'No deployable payload found');
        ...
        for (var i = 0; i < payload.calls.length; i++) {
          ...
          if (!result.success) {
            if (result.status === 409) {
              setDeploymentStatus(pol.id, 'exists', 'Already exists in tenant');
              showToast(pol.id + ': already exists');
              return { success: false, status: 'exists', error: result.error };
            }
            setDeploymentStatus(pol.id, 'failed', result.error);
        ...
        if (spoPayload.calls.length === 0) {
          setDeploymentStatus(pol.id, 'failed', 'No SPO Graph payload for ' + pol.id);
        ...
        for (var si = 0; si < spoPayload.calls.length; si++) {
          ...
          if (!spoResult.success) {
            setDeploymentStatus(pol.id, 'failed', spoResult.error);
        ...
        if (exoExtractor.commands.length === 0) {
          setDeploymentStatus(pol.id, 'failed', 'No deployable commands');
        ...
        for (var ei = 0; ei < exoExtractor.commands.length; ei++) {
          ...
          if (!exoResult.success) {
            setDeploymentStatus(pol.id, 'failed', cmd.cmdletName + ': ' + exoResult.error);
        ...
        for (var pi = 0; pi < pvExtractor.commands.length; pi++) {
          ...
          if (!pvResult.success) {
            setDeploymentStatus(pol.id, 'failed', pvCmd.cmdletName + ': ' + pvResult.error);
      ...
      setDeploymentStatus(pol.id, 'success', 'Deployed successfully');
      AppState.addDeploymentRecord({ policyId: pol.id, status: 'success', type: pol.type, detail: 'Deployed successfully' });
      showToast(pol.id + ' deployed successfully');
      return { success: true };
    } catch (err) {
      setDeploymentStatus(pol.id, 'failed', err.message);
      AppState.addDeploymentRecord({ policyId: pol.id, status: 'failed', type: pol.type, detail: err.message });
  ...
  async function deployBulk(policyIds) {
    // Sort by dependency order if DependencyGraph is available
    if (typeof DependencyGraph !== 'undefined' && DependencyGraph.suggestOrder) {
      policyIds = DependencyGraph.suggestOrder(policyIds);
    }
  ...
  async function deployBulkWithJson(entries) {
    // Sort entries by dependency order if DependencyGraph is available
    if (typeof DependencyGraph !== 'undefined' && DependencyGraph.suggestOrder) {
      var orderedIds = DependencyGraph.suggestOrder(entries.map(function (e) { return e.id; }));
      var entryMap = {};
      entries.forEach(function (e) { entryMap[e.id] = e; });
      entries = orderedIds.map(function (id) { return entryMap[id]; }).filter(Boolean);
    }
    const results = { total: entries.length, succeeded: 0, failed: 0, exists: 0 };
    AppState.set('deploymentProgress', { ...results, completed: 0 });
    for (let i = 0; i < entries.length; i++) {
      const result = await deploySinglePolicyWithJson(entries[i].id, entries[i].rawJson);
      if (result.success) results.succeeded++;
      else if (result.status === 'exists') results.exists++;
      else results.failed++;
      AppState.set('deploymentProgress', { ...results, completed: i + 1 });
      if (i < entries.length - 1) await new Promise(function (r) { setTimeout(r, 300); });
    }
    AppState.set('deploymentProgress', null);
    showToast('Deployed: ' + results.succeeded + ' success, ' + results.exists + ' existing, ' + results.failed + ' failed');
    return results;
  ...
  return {
    ...
    extractPayload,
    // Execution
    deploySinglePolicy, deploySinglePolicyWithJson, deployBulk, deployBulkWithJson,
