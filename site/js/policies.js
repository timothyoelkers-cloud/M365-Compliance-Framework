...
const Policies = (() => {
  ...
  function viewDetail(id) {
    ...
    if (scanResult) {
      ...
      // View Diff button
      if (typeof ScanDiff !== 'undefined') {
        scanHtml += `<button class="btn btn-sm" style="margin-top:4px" onclick="ScanDiff.showDiffModal('${pol.id}')">View Setting Comparison</button>`;
      }
    }
    ...
    <div class="modal-body">
      ...
      ${typeof DependencyGraph !== 'undefined' ? DependencyGraph.renderDependencyView(pol.id) : ''}
      ${typeof DependencyGraph !== 'undefined' ? DependencyGraph.renderPrereqWarning(pol.id) : ''}

      ${renderDeployHistory(pol.id)}
  ...
  async function deploy(id) {
    ...
    await PreDeployConfig.interceptDeploy(id);
    render();
  }
  ...
  async function deploySelected() {
    ...
    // Route through BulkConfig for merged configuration
    if (typeof BulkConfig !== 'undefined') {
      await BulkConfig.startBulk(ids, 'deploy');
      return;
    }
  ...
  async function downloadScriptBundle() {
    ...
    // Route through BulkConfig for merged configuration
    if (typeof BulkConfig !== 'undefined') {
      await BulkConfig.startBulk(policies.map(p => p.id), 'script');
      return;
    }
  ...
  function renderDeployHistory(policyId) {
    var history = AppState.getDeploymentHistory ? AppState.getDeploymentHistory(policyId) : [];
    if (history.length === 0) return '';
    var html = '<div class="section-hdr" style="margin-top:16px">Deployment History</div>';
    ...
    shown.forEach(function (h) {
      ...
      html += '<div class="deploy-history-item">' +
        '<span class="deploy-history-ts">' + escHtml(ts) + '</span>' +
        '<span class="deploy-history-status ' + h.status + '">' + h.status + '</span>' +
        '<span style="font-size:.66rem;color:var(--ink3)">' + escHtml(h.detail || '') + '</span>' +
        '</div>';
    });
    if (history.length > 10) {
      html += '<div style="font-size:.62rem;color:var(--ink4);padding:4px 0">+ ' + (history.length - 10) + ' more entries</div>';
    }
    return html;
  }
